...
module.exports = function(db, config){
	...
	var methods = {
		...
		getAllBeeCards: function(){
			return new Promise(function(resolve, reject){
				db.BEES_CARDS.find({}).sort({date: -1}).exec(function(err, docs){
		...
		isBeeRegistered: function(data, callback){
				return new Promise(function(resolve, reject){
					db.BEES_CARDS.find({from: data.from}, function(err, doc){
						if (err) return reject(err);
						if (doc.length !== 0){
							// already exists
							resolve(true);
							...
							resolve(false);
							// db.BEES_CARDS.insert(data, function(err, newDoc){
							// 	if (err) return reject(err);
							// 	resolve(newDoc);
							// });
						}
		...
		putBeeCard: function(data, callback){
				return new Promise(function(resolve, reject){
					db.BEES_CARDS.insert(data, function(err, newDoc){
						if (err) return reject(err);
						resolve(newDoc);
