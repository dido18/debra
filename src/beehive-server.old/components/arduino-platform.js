...
var RadioHead433mhz = function(board){

	var type;   // rpi | arduino
	// serialport module
	...
	function _constructor(){
		if (typeof board === 'undefined') throw new Error('No parameter passed to RadioHead433mhz class');
	...
	function choose_port(whatToUse){
		...
		serialPort.list(function (err, ports) {
			...
			if (argv.serialport || process.env.SERIAL_PORT){
				...
				var classe = new RadioHead433mhz({platform: whatToUse, port: port});
				...
				prompt.get({properties: {port: {required: true, type: 'number', conform: function(value){
		    		...
		    		// return choosen port
		    		var classe = new RadioHead433mhz({platform: whatToUse, port: ports[result.port-1].comName});
