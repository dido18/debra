...
module.exports = function(io, serial){ // rf433mhz){//, rf433mhz, dbFunctions){
		...
		onConnection: function(client_socket){ // single user socket
           ...
           //  methods.asyncEmitInitCards(client_socket);
            ...
            client_socket.on('command', function(data){
              ...
              if (serial.isOpen()){
          				serial.write(String(data), function (error) {
                    if (error) console.log("error in write");
                    else console.log("Sent "+ JSON.stringify(data));
                    });
          		}else{
          				console.log('SerialPort not open!');
          			}

            }); //_methods.onDeviceShake);
